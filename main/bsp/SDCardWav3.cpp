...
//    I2S
          ...
          #define Ultra_in      34          // Ultrasonic Input GPIO
          #define mic_in        39          // Microphone Input GPIO
          #define power_in      35          // Power Input GPIO
...
void setup() {    
    Serial.begin(115200);
    delay(1000);                               // Used for info/debug
    SDCardInit();
...
{    
  ...
  bool flag = false;
  if (digitalRead(Ultra_in) == HIGH)
  {
    SetWavFile("/Ultrasonic.wav");
    while(true)
    {
      if (flag == true)
      {
  ...
  if (digitalRead(mic_in) == HIGH)
  {
    SetWavFile("/loudnoise.wav");
    while(true)
    {
      if (flag == true)
      {
...
void PlayWav(bool& flag)
{
  ...
  {
    BytesRead=ReadFile(Samples, flag);                      // Read data into our memory buffer, return num bytes read in
...
uint16_t ReadFile(byte* Samples, bool& flag)
{
    ...
    {
      // If next read will go past the end then adjust the 
...
bool FillI2SBuffer(byte* Samples,uint16_t BytesInBuffer)
{
    ...
    // To make the code easier to understand I'm using variables for some calculations. Normally I'd write these calcs
    // directly into the line of code where they belong, but this makes it easier to understand what's happening
...
{
  WavFile = SD.open(Wav);        // Open the wav file
  if(WavFile==false)
  {
  ...
  {
    WavFile.seek(0);                         // Reset to the beginning before reading header                       
    WavFile.read((byte *) &WavHeader,44);    // Read in the WAV header, which is first 44 bytes of the file.
    DumpWAVHeader(&WavHeader);               // Dump the header data to serial                                     
